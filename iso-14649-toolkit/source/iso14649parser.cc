 ...
 int spaces)             /* number of spaces starting line */
{
  ...
  printf("axis2placement3d #%d '%s'\n",
	 sys->get_id()->get_val(), sys->get_name());
 ...
 int spaces)    /* number of spaces starting line */
{
  ...
  for (n=0; n < spaces; n++)
    putchar(' ');
  printf("boss #%d '%s'\n",
	 feat->get_id()->get_val(), feat->get_itsId());
}
 ...
 int spaces)          /* number of spaces starting line */
{
  ...
  for (n=0; n < spaces; n++)
    putchar(' ');
  printf("cartesianPoint #%d '%s' (", pt->get_id()->get_val(), pt->get_name());
 ...
 int spaces)         /* number of spaces starting line */
{
  ...
  for (n=0; n < spaces; n++)
    putchar(' ');
  printf("direction #%d '%s' (", dir->get_id()->get_val(), dir->get_name());
  for (iter = coords->begin(); ; )
    {
      printDouble((*iter)->get_val());
      if (++iter == coords->end())
 ...
 int spaces)                 /* number of spaces starting line */
{
  ...
  printf("drilling operation #%d '%s'\n", 
	 (dynamic_cast<instance *>(op))->get_id()->get_val(),
	 op->get_itsId());
}
 ...
 int spaces)                    /* number of spaces starting line */
{
  ...
  printf("machiningWorkingstep #%d '%s'\n", 
	 step->get_id()->get_val(), step->get_itsId());
  ...
  if (feat->isA(region_E))
	...
	putchar(' ');
      printf("region #%d '%s' CANNOT HANDLE REGION\n", 
	     (dynamic_cast<instance *>(feat))->get_id()->get_val(),
	     feat->get_itsId());
    }
 ...
 int spaces)          /* number of spaces starting line */
{
  ...
  for (n=0; n < spaces; n++)
    putchar(' ');
  printf("ncFunction #%d \n", 
	 (dynamic_cast<instance *>(func))->get_id()->get_val());
}
 ...
 int spaces)               /* number of spaces starting line */
{
  ...
  printf("owner ");
  if (owner)
    printf("#%d '%s'\n", owner->get_id()->get_val(), owner->get_itsPerson());
  else
    printf(" NONE\n");
}
 ...
 int spaces)     /* number of spaces starting line */
{
  ...
  for (n=0; n < spaces; n++)
    putchar(' ');
  printf("%s plane #%d '%s'\n", use,
	 plain->get_id()->get_val(), plain->get_name());
 ...
 int spaces)                /* number of spaces starting line */
{
  ...
  printf("programStructure #%d \n", 
	 (dynamic_cast<instance *>(struc))->get_id()->get_val());
}
...
{
  printf("project #%d '%s'\n", proj->get_id()->get_val(), proj->get_itsId());
 ...
 int spaces)     /* number of spaces starting line */
{
  ...
  for (n=0; n < spaces; n++)
    putchar(' ');
  printf("setup #%d '%s'\n", set->get_id()->get_val(), set->get_itsId());
 ...
 int spaces)         /* number of spaces starting line */
{
  ...
  for (n=0; n < spaces; n++)
    putchar(' ');
  printf("workpiece #%d '%s'\n", work->get_id()->get_val(), work->get_itsId());
}
 ...
 int spaces)        /* number of spaces starting line */
{
  ...
  for (n=0; n < spaces; n++)
    putchar(' ');
  printf("workplan #%d '%s'\n", plan->get_id()->get_val(), plan->get_itsId());
